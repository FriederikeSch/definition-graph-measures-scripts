%Das Programm erstellt eine Walkmap die einen RandomWalk enthält.
%In: PartList, graph.mat Files
%Out: walkmap eines RandomWalk
...
for ii = 1:number
    ...
    elseif exist(file)==2
       ...
       cd '/Users/User/Documents/Studium/BA/StartingPackage Graph Theory Kopie/Scripts/Meine Skripte';


       % Random Walk von einem zufälligen Startknoten bis ein Zielknoten gefunden wird
       % für maximal x Schritte. Erstellt eine walkmap in welcher Reinfolge
       % die Knoten besucht wurden. Für einen RandomWalk ohne Zielknoten 
       %die if Bedingung entfernen.
       
        num=randi([1,length(nodes)]);
      
        %ending=3;
        steps=1000;
        %der RandomWalk macht maximal  Schritte
        for i=1:steps
        
            num=randomwalkfun(graphy,num);
