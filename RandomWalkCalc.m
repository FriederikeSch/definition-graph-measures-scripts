%Das Programm sortiert die Daten aus der RandomWalkBetweenness Liste nach
%den Häusern.
%In: randwalkbetnodelist enthält die RandomWalkBetweenness Werte für die
%einzelnen Graphen. Jede Spalte enthält einen Graphen. Die Zeilen jeweils
%die Werte für die Häuser des Graphen
%    PartList und graph.mat files
%Out: randwalkbetcompletelist enthält gesammelte die RandomWalkBetweenness für die
%einzelnen Häuser
...
load('HouseList');
load ('newfullgreatlist');
total=zeros(1,213);
randwalkbetcompletelist=zeros(1,213);
...
for ii= 1:Number
    ...
    % analysis
        ...
        %die RandomWalkBet für je einen Graphen
        row=randwalkbetnodelist(ii,:);
        
        %enthält die Position in der RandWalkListe
        counter=1;
        
        numn=numnodes(graphy);
        
        
        %die RandWalkListe enthält an ihrem Ende Leerstellen um die
        %unterschiedlich großen Reihen auszugleichen diese werden gelöscht
        ...
        %member ist ein logisches Array das zeigt welche Häuser im Graphen
        %enthalten sind
        ...
        for pos=1:length(houseList)
            ...
            if member(pos,1)==1
                ...
                h= randwalkbetcompletelist(1,pos);
                
                %ist der neue Wert aus dem Graphen
                graphvalue=  rowreduced(1,counter);
                %der Counter wird auf das nächste vorhandene Haus im
                %Graphen gesetzt
                counter=counter+1;
                
                %der Wert -1 zeigt an dass das Haus zwar im Grraphen liegt
                %aber auf dem RandomWalk nicht besucht wurde also auch kein
                %Wert vorhanden ist
                if graphvalue==-1
                   
                else
                    %der Wer aus dem Graph wird normalisiert, sodas er 
                    %zwischen 0 und 1 liegt
                    %die beiden Werte werden zusammen gerechnet und der
                    %neue Wert gespeichert
                    
                   graphvalue=graphvalue/(((numn-1)*(numn-2))/2);
                   
                   
                   randwalkbetcompletelist(1,pos)=(h+graphvalue);
                   hold= total(1,pos);
                   hold=hold+1;
                   total(1,pos)=hold;
...
for j=1:213
    ...
    randwalkbetcompletelist(1,j)=randwalkbetcompletelist(1,j)/total(1,j);
