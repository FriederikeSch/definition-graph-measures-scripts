%The program calculates the assortative coefficient of nodes for the different
%graphs. The assortative coefficient shows if nodes with high degrees tend
%to connect to other graphs with high degree or to ones with low degree.
%The equation used to calculate the coefficient is found at: Assortative Mixing Networks by M.E.J. Newman
%https://journals.aps.org/prl/pdf/10.1103/PhysRevLett.89.208701
%
%In: PartList, graph.mat files 
%Out: sumlist: a list of the assortative coefficient for all participants
 ...
 for ii = 1:Number
    ...
    elseif exist(file)==2
        ...
        for i=1:numedge
            ...
            [in,out]=findedge(graphy,i);
         
            degj=degree(graphy,in);
            degk=degree(graphy,out);
            
            sumone=degj*degk+sumone;
            sumtwo= (0.5*(degj+degk))+sumtwo;
            sumthree= (0.5*((degj^2)+(degk^2)))+sumthree;
            
        end
        
        termone=M*sumone;
        termtwo=(M*sumtwo)^2;
        termthree=(M*sumthree);
        
        counter=termone-termtwo;
        denominator=termthree-termtwo;
        
        sumtot=counter/denominator;
        sumlist(ii)=sumtot;
