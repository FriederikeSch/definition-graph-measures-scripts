%Berechnet Degree, Closeness, Shortest Path Betweeenness und 
% Eigenvector Centrality Werte für jedes Haus 
% nutzt dafür die "centrality" Funktion
%
% In: PartList, graph.mat files
% Out: centlist, eine Liste mit den Centrality Werten für jedes Haus
clearvars;
...
for ii = 1:Number
    ...
    % analysis
        ...
        numn=numnodes(graphy);
        
        degc=transpose(centrality(graphy,'degree'));
        closec=transpose(centrality(graphy,'closeness'));
        betwc=transpose(centrality(graphy,'betweenness'));
        eigvc=transpose(centrality(graphy,'eigenvector'));
        
         member=ismember(houseList,nodes);
         counter=1; 
         
        %Degree Centrality 
        for pos=1:length(houseList)
            
            
            %wenn das Haus im Graphen vorkommt    
            if member(pos,1)==1
                
                %ist der bereits berechneten Wert für das Haus
                deglist= centlist(1,pos);
                
                %ist der neue Wert aus dem Graphen
                deg=  degc(1,counter);
                %der Counter wird auf das nächste vorhandene Haus im
                %Graphen gesetzt
                counter=counter+1;
                ...
                if deglist==0
                    ...
                    %die beiden Werte werden zusammen gerechnet und der
                    %neue Wert gespeichert
                    %die gesamt Anzahl an für dieses Haus verwendeten
                    %Werten wird um 1 erhöht
                    centlist(1,pos)=(deglist+deg);
        ...
        %Closeness Centrality
            ...
            %wenn der Knoten im Graphen vorkommt    
            if member(pos,1)==1
                
                %ist der bereits berechnete Centraliyty Wert
                closelist= centlist(2,pos);
                
                %ist der neue Wert des Graphen
                close=  closec(1,counter);
               
                %der Counter wird auf den nächsten vorhandenen Knoten im
                %Graphen gesetzt
                counter=counter+1;
                
                %wenn das Haus noch keinen Wert hat wird der Wert aus dem
                %Graphen übernommen
                if closelist==0
                    centlist(2,pos)=close;
                else
                
                    %die beiden Werte werden zusammen gerechnet und der
                    %neue Wert gespeichert
                    centlist(2,pos)=(closelist+close);
             ...
             % Shortest Path Betweennes Centrality    
            for pos=1:length(houseList)
            ...
            if member(pos,1)==1
                ...
                %ist der bereits berechnete Centraliyty Wert
                betwlist= centlist(3,pos);
                
                %ist der neue normalisierte Wert des Graphen
                betw=  betwc(1,counter);
                
                betw=betw/(((numn-1)*(numn-2))/2);
            ...
            if member(pos,1)==1
                ...
                %ist der bereits berechnete Centraliyty Wert
                eigenvlist= centlist(4,pos);
                
                %ist der neue Wert des Graphen
                eigenv=  eigvc(1,counter);
                ...
                %Graphen übernommen
                if eigenvlist==0
                    centlist(4,pos)=eigenv;
                else
                
                    %die beiden Werte werden zusammen gerechnet und der
                    %neue Wert gespeichert
                    centlist(4,pos)=(eigenvlist+eigenv);
...
for j=1:213
    ...
    centlist(:,j)=centlist(:,j)/total(1,j);
