%Random Walk von A über B nach C dargestellt als Möglichkeitswürfel
%
%In: PartList, graph.mat files, walkmap (aus Random Walk)
%Out: cube mit den Informationen wie oft jede Kombination vorkam
...
for ii = 1:Number
    ...
    elseif exist(file)==2
        ...
        %one der Knoten wurde eingeschaltet
        one=true;
        %der Knoten ist aus
        zero=false;
        
        %das Anfangstriplett ausgeschaltet
        offtrip3={zero,1,0};
        
        
        %cube erstellen und füllen
        cube(1:len,1:len,1:len)={offtrip3};
        
     
        for r=1:length(walkmap)
        
        %num ist der Knoten der betrachtet wird
        num=walkmap(r);
        
        %A 
        sliceA=cube(:,num,:);
        
        %die Funktion findfirst steht am Ende des Skripts sie ändert den
        %ersten Wert im Triplet auf 1 (sie schaltet alle 'von' Punkte ein)
        sliceA=cellfun(@findfirst,sliceA,'UniformOutput',false);
        
        %die neuen Werte werden in den Cube zurück geschrieben
        cube(:,num,:)=sliceA;
         
        %B
        sliceb=cube(:,:,num);
        
        sliceb=cellfun(@copyfirst,sliceb,'UniformOutput',false);
        ...
        slicec=cube(num,:,:);
        slicec=cellfun(@copysecond,slicec,'UniformOutput',false);
...
% Kopiert den Wert aus der zweiten Position in die dritte Position
% Resetet das Triplett
function [cell]=copysecond(cell)
...
% kopiert den Werte aus der ersten Position in die zweite
%(schaltet die zweite Position ein)
 function [cell]=copyfirst(cell)
 
     cell(2)=cell(1);
 end
 
 % Schaltet die die erste Position im Triplet ein
function [cell]=findfirst(cell)
