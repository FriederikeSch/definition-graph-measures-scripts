...
% %The skript calculates the global clustering coefficient (GCC) for all
% %participants. It uses the definition of the GCC from Clustering in
% %weighted networks by Tore Opsahl and Pietro Panzarasa. 
% %The GCC is the ratio of all closed triplets by the number of all open
% %triplets
% 
% %Input: Participant List, Graph.mat files
% %Output: gccall: a List of the GCC for all Participants 
% %        gccaver: the average GCC over all participants
% 
 clear all;
...
% %PartList={5839,8437,1002};
% number = length(PartList);
% 
% %List of the global clustering coefficient for each Participant
% gccall={};
% %Holds the average GCC over all partcipants
% gccaver=0;
% 
% 
% for ii = 1:number
...
graphy=g;
       ...
       %A triplet is a connection from one node to two different other
       %nodes. It is closed when the two other nodes are also connected.
       closedtrip=0;
       %The number of open triplets
       opentrip=0;
       
       %A list of all the nodes in each Graph
       ...
       for nodenumber=1:length(nodes)
           ...
           for neighbourcount=1:length(neighbourlist1)
               ...
               neighbourlist2=neighbors(graphy,neighbourlist1(neighbourcount));
               ...
               for y=1:length(neighbourlist1)
                    ...
                    elseif ismember(neighbourlist1(y),neighbourlist2)==true
                        closedtrip=closedtrip+1;
                   
                    %if it is not a member they build an open triplet    
                    elseif ismember(neighbourlist1(y),neighbourlist2)==false
                            opentrip=opentrip+1;
                    else
      ...
      alltrip=opentrip+closedtrip;
       
       % the global clustercoefficient 
       gcc= closedtrip/alltrip;
       
       %collect the GCC for all participants
      % gccall{1,ii}=gcc;
...
%    end
%end
%calculates the average GCC over all participants
%gccaver=gccaver/312;

%plots the cluster coefficient for all participants
%plotclusterall=cell2mat(clustercoeffall);
%plot(plotclusterall);
