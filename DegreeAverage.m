%Berechnet die durschnittlichen Degrees für die Häuser für aller
%Participants  
%Berechnet wie viele Häuser jeder Participant gesehen hat
%
% In: PartList, graph.mat files, HouseList, housecounterlist
% Out: degaver, enthält den durschnittlichen Degree eines Houses
%      housesseen, enhält wie oft ein Haus gesehen wurde
...
load('HouseList');
load('housecounterlist');
degaver=zeros(1,213);
houseseen=zeros(1,312);
...
for ii = 1:Number
    ...
    elseif exist(file)==2
        ...
        d=degree(graphy);
 %%  Berechnet Diameter und Radius     
       dist=distances(graphy);
       
       diameter(1,ii)=max(dist,[],'all');
       
       averdim=mean(diameter);
       
       rows= max(dist);
       
       [radius(2,ii),radius(1,ii)]=min(rows);
...
%%
        ...
        for i=1:length(nodes)
           ...
           j= find (strcmp(houseList,nodes(i,1)));
           degaver(1,j)=degaver(1,j)+d(i,1);
        end
 ...
 for i=1:213
     degaver(1,i)=degaver(1,i)/housecounter(1,i);
 end
